    ...
    <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <h2 className="text-xl md:text-2xl font-extrabold text-[#ffe9b3]">
          International Education Award, 2026 – {currentYear}
        </h2>
      </div>
      <div className="flex flex-wrap gap-2 text-xs">
        {editions.map((edition) => {
          const formattedTitle = edition.title?.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "global-international education-awards";
