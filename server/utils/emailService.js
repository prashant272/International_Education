...
export const sendOTPEmail = async (email, otp) => {
    const mailOptions = {
        from: `"International Education Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Verify Your Email - International Education Awards",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #d4af37; border-radius: 10px;">
        <h2 style="color: #d4af37; text-align: center;">Welcome to International Education Awards</h2>
        <p>Dear User,</p>
        ...
        <hr>
        <p style="font-size: 12px; text-align: center;">© International Education Awards 2026</p>
      </div>
    `,
    };
...
export const sendNominationSuccessEmail = async (email, nomineeName, credentials = null) => {
    ...
    const mailOptions = {
        from: `"International Education Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: credentials ? "Nomination Received & Account Created - International Education Awards 2026" : "Nomination Received - International Education Awards 2026",
        html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #d4af37; border-radius: 15px; background-color: #ffffff;">
        ...
        <p style="font-size: 12px; color: #888; text-align: center; margin-bottom: 0;">
            Sent by <strong>International Education Awards & Summit 2026</strong><br>
...
export const sendPasswordResetEmail = async (email, otp) => {
    const mailOptions = {
        from: `"International Education Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Password Reset Code - International Education Awards",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #d4af37; border-radius: 10px;">
...
export const sendNominationCredentialsEmail = async (email, name, password) => {
    const mailOptions = {
        from: `"International Education Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Your Account Credentials - International Education Awards 2026",
        html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #d4af37; border-radius: 15px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 25px;">
            <h1 style="color: #d4af37; margin: 0; font-size: 28px;">Welcome to International Education Awards!</h1>
        </div>
        ...
        <p style="font-size: 12px; color: #888; text-align: center; margin-bottom: 0;">
            © International Education Awards 2026<br>
            Organised by Prime Time Research Media Pvt. Ltd.
        </p>
      </div>
    `,
